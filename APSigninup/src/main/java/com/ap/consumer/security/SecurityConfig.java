...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable()
			.authorizeRequests()
			//로그인 관련 화면 접속 허용
			.antMatchers("/users/loginForm","/users/login","/users/").permitAll()
			.antMatchers("/users/loginForm?error").permitAll()
			//회원가입 관련 화면 접속 허용
			.antMatchers("/users/signup", "/users/signupForm").permitAll()
			//아이디/비밀번호 찾기 관련 화면 접속 허용
			.antMatchers("/find/findForm","/find/findId", "/find/findPw").permitAll()
			//메인화면 및 이외의 접속으로 넘어가려면 로그인 필요
			.antMatchers("/users/success").authenticated()
